...
typedef struct {
        net_handle_t nh;
        buffer_t buf;
} sdevent_queue_schedule_ctx_t;

static void __sdevent_queue_buf_exec(void *_arg)
{
        sdevent_queue_schedule_ctx_t *arg = _arg;

        __sdevent_queue(&arg->nh, &arg->buf, 0);
        yfree((void **)&arg);
}

//这个地方有隐患，虽然const buffer_t *buf， 但是会被修改
static int __sdevent_queue_buf(const net_handle_t *nh, const buffer_t *buf)
{
        int ret;
        sdevent_queue_schedule_ctx_t *arg;
        schedule_t *schedule;

        if (schedule_running()) {
                ret = ymalloc2((void **)&arg, sizeof(sdevent_queue_schedule_ctx_t));
                if (unlikely(ret))
                        GOTO(err_ret, ret);

                arg->nh = *nh;
                mbuffer_init(&arg->buf, 0);
                mbuffer_merge(&arg->buf, (void *)buf);

                schedule_task_new("sdevent_queue_buf", __sdevent_queue_buf_exec, arg, -1);
                schedule = schedule_self();
                schedule_post(schedule);
        } else {
                return __sdevent_queue(nh, buf, 0);
        }
...
int sdevent1_queue(const net_handle_t *nh, job_t *job)
{
        ...
        job->nethash = nh->u.sd.sd % gloconf.sdevents_threads;
        job->sock = nh->u.sd;

        ret = __sdevent_queue_buf(nh, job->iocb.buf); 
        if (ret)
                GOTO(err_ret, ret);

        if (job->iocb.op == KEEP_JOB) {
                DBUG("job %s sent %p\n", job->name, job->iocb.sent);
                if (job->iocb.sent)
                        job->iocb.sent(job);
                job->in_wait = 1;
        } else {
