...
void nodepool_hash_print_hdd(void)
{
        struct node_pool *np;
        np = &mds_info.nodepool_hdd;
...
int nodepool_destroy_hdd(struct node_pool *np, int group)
{
        (void) np;
        (void) group;
...
int nodepool_init_hdd()
{
        struct node_pool *np = &mds_info.nodepool_hdd;
...
int nodepool_addisk_hdd(const uuid_t *_nodeid, const diskid_t *diskid)
{
        int ret, found;
...
static int nodepool_reset_hdd(void)
{
        struct list_head *pos;
        struct list_head *q;
        struct node_pool *np;
...
int nodepool_get_hdd(uint32_t reps, net_handle_t *disks, int hardend)
{
        int ret, dup;
        uint32_t i, j;
        struct node_pool *np;
 ...
 */
int nodepool_nodedead_hdd(diskid_t *diskid, uuid_t *nodeid)
{
        int ret;
        struct node_pool *np;
        struct node_info *ni;
...
int nodepool_diskdead_hdd(const diskid_t *diskid)
{
        int ret;
...
*Date   : 2017.04.18
*Author : Yang
*nodepool_get_node_num_hdd : get hdd node number of NAS
*/
int nodepool_get_node_num_hdd(uint32_t *node_num)
{
        int ret;
