...
int main(int argc, char *argv[])
{
        ...
        //XXX debug
        yrecycle_fd = open(YRECYCLE_LOG, O_APPEND | O_CREAT | O_WRONLY, 0644);
        if (yrecycle_fd == -1) {
                ret = errno;
...
#if 0
        ret = recursion_rmdir(FAKE_UNLINK_PREFIX, verbose, delete_all, month); 
        if (ret) {
                fprintf(stderr, "recursion rmdir(%s) %s\n", 
                                FAKE_UNLINK_PREFIX, strerror(ret));
                exit(1);
        } 
#endif
                ...
                while (delen > 0) {
                        ...
                        if (S_ISDIR((stbuf).st_mode)) {
                                ...
                                if (del_all || (strncmp(daynow, de->d_name, _strlen(daynow))== 0 && !del_all)) {
                                        ...
                                        if (ret) {
                                                fprintf(stderr, "Error: recursion_rmdir %s error: %s",
                                                                depath, strerror(ret));
                                                goto next;
                                        }
