...
#include "configure.h"
#include "sdfs_lib.h"
#include "../../yfs/objc/objc.h"
#include "network.h"
#include <ctype.h>
#include "f_name_client_cache.h"
...
static int yfs_getattr(const char *_path, struct stat *stbuf, struct fuse_file_info *fi)
{
        int ret;
...
static int yfs_mkdir(const char *_path, mode_t mode)
{
        ...
        ec.k = 0;
        ec.m = 0;
        ec.plugin = PLUGIN_NULL;
        ret = ly_mkdir(path, &ec, mode);
        if (unlikely(ret))
                GOTO(err_ret, ret);
...
static int yfs_rename(const char *_from, const char *_to, unsigned int flags)
{
        int ret;
...
static int yfs_chmod(const char *_path, mode_t mode, struct fuse_file_info *fi)
{
        int ret;
        char path[MAX_PATH_LEN];
...
static int yfs_chown(const char *_path, uid_t uid, gid_t gid, struct fuse_file_info *fi)
{
        int ret;
        char path[MAX_PATH_LEN];
...
static int yfs_truncate(const char *_path, off_t size, struct fuse_file_info *fi)
{
        int ret;
        char path[MAX_PATH_LEN];
...
static int yfs_utimens(const char *_path, const struct timespec ts[2], struct fuse_file_info *fi)
{
        int ret;
        char path[MAX_PATH_LEN];
...
void usage()
{
    fprintf(stderr, "usage: uss_fuse3 [-f][--allow_other] --dir=remotedir  mountpoint  --service=nums\n");
